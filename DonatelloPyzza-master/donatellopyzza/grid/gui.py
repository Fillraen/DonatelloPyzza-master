...
class GUI:
    ...
    """
    Goal : update only one square in the GUI (not the whole grid)
    """
    def updateOneSquare(self, square):
        #replace last square by the new one
        print('Last square : ', square)
        print('Displayed squares : ', self.squareToDisplay)
        #actualSquare = self.squareToDisplay[square]
        #print('Actual square : ', actualSquare)
        """
        actualSquareColor = actualSquare[0]
        actualSquareCoord = actualSquare[1]
        actualSquareText = actualSquare[2]
        """
        pass
